...
El 2026-05-12 se incorporo la carpeta `documentacion/documentacion_tecnica/base_datos/` con documentos del modelo de base de datos:
...
El diccionario describe 12 tablas principales: `REGIONALES`, `PAG`, `DEFENSORES`, `ASIGNACION`, `PERSONA`, `SITUACION_CARCELARIA`, `CALIFICACION_CONDUCTA`, `GESTION_JURIDICA`, `PONAL`, `SISIPEC`, `AURORA_10` y `LOG_CARGA`. El manual indica despliegue sobre el esquema `DNDP` y menciona objetos adicionales como `VW_DETALLE_CON_DEFENSOR` y procedimientos ETL (`PRC_CARGA_PONAL`, `PRC_CARGA_SISIPEC_V3`, `PRC_CARGA_AURORA10`). En el ambiente de desarrollo está disponible el nombre `PRC_CARGA_SISIPEC`, por lo que el modulo de cargas lo usa por defecto y permite sobreescribirlo con `CARGUEBD_SISIPEC_PROCEDURE` si otro ambiente conserva `_V3`. El 2026-05-19 `DNDP.PRC_CARGA_SISIPEC` fue recompilado a `VALID` y la carga completa de SISIPEC termino correctamente. La carpeta `documentacion/documentacion_tecnica/base_datos/` no incluye un archivo ejecutable `BD.sql`; por eso el setup de pruebas creado en el repositorio arma desde codigo las 12 tablas documentadas y carga una semilla pequena para que las pruebas del backend tengan registros reales de trabajo.

En la misma validacion se confirmo que las 12 tablas base y los 3 procedimientos ETL estan `VALID` en desarrollo, pero las vistas `VW_DETALLE_CON_DEFENSOR` y `VW_PPL_CON_DECISION` quedaron `INVALID` porque referencian columnas antiguas de `GESTION_JURIDICA` (`DEFENSOR`, `CEDULA_PAG`). El backend actual usa joins equivalentes desde repositorios Oracle y obtiene defensor/PAG desde `ASIGNACION`; si esas vistas se vuelven a usar como fuente oficial, deben recrearse con el modelo vigente.

## 9.1) Cargas staging y ETL

El repositorio incluye el modulo `scripts/cargas_bd/` para cargar archivos Excel mensuales en tablas staging (`PONAL`, `SISIPEC`, `AURORA_10`) y ejecutar los procedimientos ETL documentados. Desde el 2026-05-19, esa capacidad tambien esta expuesta desde Aurora mediante la vista administrativa `Cargas mensuales` y rutas `/api/admin/cargas`.

La documentacion operativa del flujo, variables, rutas y validaciones esta en `documentacion/soporte/operacion/16_cargas_staging_etl_bd.md`.
