...
|---|---|---|
| Servicio Python | `scripts/cargas_bd/loader_service.py` | Lee Excel, valida formato, carga staging y ejecuta ETL. |
| Wrappers Python | `scripts/cargas_bd/cargar_aurora_10.py`, `cargar_sisipec.py`, `carga_ponal_oracle.py` | Mantienen ejecucion por consola compatible con los scripts historicos. |
| Dependencias Python | `scripts/cargas_bd/requirements.txt` | Lista `pandas`, `openpyxl` y `oracledb`. |
...
6. El backend inicia `scripts/cargas_bd/loader_service.py` en segundo plano.
...
La carpeta `scripts/cargas_bd/` contiene únicamente el servicio Python, wrappers y dependencias. Los archivos Excel de PONAL, SISIPEC o Aurora 1.0 no deben guardarse allí ni versionarse.
...
| `CARGUEBD_SCRIPT_PATH` | Ruta opcional al `loader_service.py` si se despliega fuera de `scripts/cargas_bd/`. |
...
```bash
python scripts/cargas_bd/cargar_aurora_10.py --archivo /ruta/Aurora_1_0.xlsx
python scripts/cargas_bd/cargar_sisipec.py --archivo /ruta/Consolidado_SISIPEC.xlsx
python scripts/cargas_bd/carga_ponal_oracle.py --archivo /ruta/CONSOLIDADO_PPL_REGIONES.xlsx
```
...
```bash
python scripts/cargas_bd/loader_service.py --fuente ponal --archivo /ruta/archivo.xlsx --no-etl
```
...
- `scripts/cargas_bd/` no debe usarse como almacenamiento de archivos mensuales; para eso se usa `AURORA_CARGAS_DIR`.
