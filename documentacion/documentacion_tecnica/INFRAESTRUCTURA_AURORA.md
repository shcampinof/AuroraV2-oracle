...
| `backend/tutorial-videos/` | Tutoriales institucionales versionados e incluidos en la imagen, con manifiesto SHA-256. |
...
La imagen no incluye `node_modules` locales, `.env*`, `frontend/dist`, `backend/public/app`, `backend/storage`, certificados, documentación ni `.cleanup-backups/`. Así se evita conservar usuarios, cargas, logs o respaldos locales en capas de la imagen. Sí incluye `backend/tutorial-videos/`, y la construcción verifica sus sumas de integridad antes de terminar.
...
| `ENABLE_STARTUP_WARMUP` | Precarga de filtros de usuarios asignados al iniciar; activa por defecto y se deshabilita con `false`. |
| `HTTPS_KEY_PATH` | Ruta de llave privada TLS, por ejemplo `certs/aurora.key`. |
| `HTTPS_CERT_PATH` | Ruta de certificado TLS o cadena completa, por ejemplo `certs/aurora.fullchain.crt`. |
...
| `AUTH_USER_ACCESS_MODE` | `open` acepta cuentas institucionales válidas; `managed` exige registro habilitado. |
| `AUTH_BOOTSTRAP_ADMIN_EMAILS` | Correos de administradores iniciales. |
| `AUTH_USER_STORE_PATH` | Archivo persistente del directorio administrado. |
| `AUTH_USER_IMPORT_MAX_MB` | Tamaño máximo del CSV de usuarios, por defecto 2 MB. |
| `AUTH_USER_IMPORT_MAX_ROWS` | Filas máximas por importación, por defecto 5000. |
| `AURORA_VIDEOS_DIR` | Directorio de tutoriales; en la imagen: `/app/backend/tutorial-videos`. |
...
| `AZURE_AD_ADMIN_GROUP_IDS` | Grupos que reciben rol interno `admin`. |
...
| `CARGUEBD_PUBLIC_ERROR_MAX_LENGTH` | Longitud máxima del detalle de error publicado por la API. |
| `CARGUEBD_REPAIR_REGISTRY_ON_START` | Repara un registro JSON corrupto al iniciar y conserva respaldo. |
| `CARGUEBD_CLEAR_REGISTRY_ON_START` | Limpia una sola vez el historial visual y conserva respaldo. |
...
Los volúmenes `aurora_auth_users` y `aurora_cargas_bd` conservan el directorio de usuarios y la operación de cargas. Los tutoriales son contenido inmutable de la imagen y no requieren volumen. Para reemplazarlos por ambiente se puede montar un directorio de solo lectura y configurar `AURORA_VIDEOS_DIR`; el montaje debe contener los tres nombres esperados por el frontend.
