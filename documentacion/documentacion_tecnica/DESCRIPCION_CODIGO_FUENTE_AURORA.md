...
Este documento describe la estructura del repositorio Aurora y sus archivos principales. Su objetivo es apoyar mantenimiento, análisis técnico y transferencia interna del proyecto.
...
| `scripts/cargas_bd/` | Servicios Python para carga de Excel a staging Oracle y ejecución ETL. |
| `documentacion/soporte/` | Documentación técnica existente. |
| `documentacion/documentacion_tecnica/base_datos/` | Documentación técnica del modelo de datos. |
...
| `documentacion/soporte/pruebas/SET_PRUEBAS_AURORA.md` | Registro de pruebas técnicas y funcionales. |
...
| `AdminUsuarios.jsx` | Directorio administrado, roles, estado e importación CSV. |
...
| `components/DataTreatmentNotice.jsx` | Exige aceptar las condiciones de tratamiento después de autenticar. |
| `components/PagUsuariosAdmin.jsx` | Administra el rol PAG desde el módulo de asignación. |
...
| `utils/actuacionesValidation.js` | Controla la actuación inicial virtual y el bloqueo de actuaciones sucesivas incompletas. |
...
- Servir los tutoriales MP4 desde `/tutorial-videos` con solicitudes Range.
...
| `routes/adminUsers.js` | Administración individual y masiva del directorio de usuarios. |
...
| `backend/services/userDirectoryService.js` | Persistencia y vigencia del directorio administrado. |
| `backend/services/userCsvImportService.js` | Análisis seguro de CSV, límites, duplicados y correos inválidos. |
| `backend/middleware/roles.js` | Autorización de operaciones PAG. |
| `backend/tutorial-videos/` | Tres tutoriales institucionales y manifiesto de integridad SHA-256. |

## Cargas staging/ETL

El directorio `scripts/cargas_bd/` contiene el motor de carga de archivos Excel mensuales hacia Oracle:
...
La carpeta `scripts/cargas_bd/` debe contener código y dependencias, no archivos Excel operativos. Los `.xlsx` se reciben por el módulo administrativo o se pasan por consola mediante `--archivo`.

La documentación completa está en `documentacion/soporte/operacion/16_cargas_staging_etl_bd.md`.
...
| `qa:encoding` | Ejecuta validación de codificación. |
...
| `test` | Ejecuta autenticación, acceso PAG, CSV, cargas, depuración y seguridad de asignaciones. |
...
El script backend `test` ejecuta los archivos `auth-config`, `pag-access`, `user-csv-import`, `carga-bd-service`, `actuacion-cleanup-service` y `asignacion-safety`.
...
- Mutaciones PAG restringidas al rol `pag`; administración de usuarios restringida a `admin`.
- Documentación fuente Markdown versionada junto al código.
- Documentación técnica previa en `documentacion/soporte/`.
...
- Archivos Excel operativos dentro de `scripts/cargas_bd/`.
- Entregables documentales binarios Word/PDF y diagramas.
