...
En producción, el backend sirve el frontend compilado. El despliegue Docker usa un contenedor único que compila `frontend/`, copia `dist` a `backend/public/app` e incorpora los tutoriales versionados en `backend/tutorial-videos/`.
...
4. Después del ingreso, la interfaz exige aceptar el aviso de tratamiento de datos para habilitar la navegación.
5. En cada petición, el backend combina el token con el directorio administrado vigente para aplicar deshabilitaciones y roles sin esperar a la expiración del JWT.
6. El frontend consulta personas privadas de la libertad, registros asignados, formatos y defensores a través de `/api`.
7. El backend accede a Oracle mediante repositorios especializados y normaliza datos de origen para exponer una respuesta estable al frontend.
8. El formulario de atención aplica reglas de negocio en cliente para visibilidad, obligatoriedad, dependencias, estado del trámite y cierre del caso.
9. Al guardar, el backend persiste los cambios funcionales y el historial de actuaciones en Oracle.
10. El módulo administrativo de cargas almacena archivos Excel en un directorio controlado, ejecuta la carga Python en segundo plano y registra estado/logs para seguimiento operativo.
...
| API usuarios | `backend/routes/adminUsers.js`, `backend/services/userDirectoryService.js` | Directorio administrado, importación CSV y vigencia inmediata de permisos. |
| Autorización PAG | `backend/middleware/roles.js` | Restringe asignación, validación PAG y creación de defensores al rol `pag`. |
...
| Tutoriales | `backend/tutorial-videos/`, `frontend/src/pages/ManualInteractivo.jsx` | Streaming local de tres MP4 con soporte HTTP Range. |
...
| `backend/tutorial-videos/` | Tutoriales incluidos en despliegues Node y Docker, verificados mediante `SHA256SUMS`. |
...
- Servicio estático `/tutorial-videos` con caché de un día, protección `nosniff` y soporte Range.
...
| Rol | Uso técnico |
|---|---|
| `user` | Acceso a rutas protegidas ordinarias: consulta, formularios, formatos e historial. |
| `pag` | Acceso a validación PAG, asignación o reasignación y creación de defensores. |
| `admin` | Administración del directorio de usuarios; puede combinarse con `pag`. |
| `carguebd` / `cargas_bd` | Operación del módulo de cargas cuando figure en `CARGUEBD_ADMIN_ROLES`. |

La ruta `/api/admin/cargas` aplica los roles configurados en `CARGUEBD_ADMIN_ROLES`; `/api/admin/users` exige `admin`; y las mutaciones PAG exigen `pag`. El directorio se guarda de forma persistente en `AUTH_USER_STORE_PATH`. Su importación CSV está acotada por `AUTH_USER_IMPORT_MAX_MB` y `AUTH_USER_IMPORT_MAX_ROWS`.
...
En desarrollo, Vite usa proxy para `/api` y `/tutorial-videos`. En producción con el backend como servidor único, se recomienda `VITE_API_BASE_URL=/api`.
...
| Usuarios autorizados | `frontend/src/pages/AdminUsuarios.jsx` | Gestión individual, roles, estado y vista previa/importación CSV. |
| Aviso de datos | `frontend/src/components/DataTreatmentNotice.jsx` | Bloquea la navegación hasta aceptar las condiciones de tratamiento. |
...
| `GET` | `/api/ppl/pag/:cedula/validar` | JWT + `pag` | Valida información PAG por cédula. |
...
| `POST` | `/api/ppl/asignar-defensor` | JWT + `pag` | Asigna defensor a registros. |
| `GET` | `/api/defensores` | JWT | Lista defensores. |
| `POST` | `/api/defensores` | JWT + `pag` | Registra defensor. |
...
| `GET/POST/PATCH/DELETE` | `/api/admin/users` | JWT + `admin` | Administra usuarios, roles y estado. |
| `POST` | `/api/admin/users/import/preview` | JWT + `admin` | Analiza CSV sin persistir. |
| `POST` | `/api/admin/users/import` | JWT + `admin` | Importa correos nuevos. |
| `GET` | `/tutorial-videos/:archivo` | Pública | Sirve tutorial con caché y solicitudes Range. |
...
- Si no existen actuaciones reales, el frontend presenta una fila virtual `Analizar el caso` y abre el registro sin crear una fila anticipada.
- En flujo condenado, una actuación adicional se bloquea cuando la última real no tiene información desde la pregunta 29.
...
- `CARGUEBD_PUBLIC_ERROR_MAX_LENGTH`
- `CARGUEBD_REPAIR_REGISTRY_ON_START`
- `CARGUEBD_CLEAR_REGISTRY_ON_START`

La depuración administrativa elimina en una sola transacción las actuaciones ficticias y asignaciones activas del defensor configurado, tras comprobar conteos esperados para evitar operar sobre datos modificados concurrentemente. Conserva personas y situaciones jurídicas y registra auditoría local.
...
- Certificados reales, llaves privadas o paquetes TLS (`.key`, `.pem`, `.crt`, `.pfx`, `.p12`).
- Hosts sensibles si la política interna lo restringe.

Para habilitar HTTPS, ubicar los certificados fuera de git, por ejemplo en `backend/certs/`, y configurar `HTTPS_KEY_PATH` y `HTTPS_CERT_PATH` en el `.env` del ambiente. Si ambas variables quedan vacías, Aurora sirve por HTTP.
...
Validaciones rápidas del artefacto Docker:

```bash
docker compose config --quiet
docker compose build aurora
docker image ls aurora-app:local
docker run --rm --entrypoint sh aurora-app:local -c \
  'cd /app/backend/tutorial-videos && sha256sum --check SHA256SUMS'
```

Si el puerto oficial `7860` está ocupado por un proceso previo de Node.js u otro servicio, se puede hacer una prueba temporal en otro puerto:
...
En esa prueba el contenedor sigue escuchando internamente en `7860`; solo cambia el puerto publicado en el servidor. Para cerrar la prueba:
...
Para reiniciar la aplicación cuando se requiere reciclar el proceso sin cambiar variables:
...
Si se cambia el archivo `.env`, recrear el contenedor para que Docker Compose tome las nuevas variables:
...
La recreación es necesaria porque Docker fija las variables de entorno al crear el contenedor. Un `docker compose restart aurora` recicla el proceso, pero no incorpora cambios nuevos del `.env`.
...
El manual asume Docker Engine instalado como servicio del sistema por una persona con permisos administrativos. Si el usuario de despliegue no tiene permisos para instalar paquetes, habilitar servicios o abrir puertos, la actividad debe escalarse a Infraestructura antes de continuar.

### Autenticación institucional y Azure

Aurora expone la configuración pública de autenticación en:

```bash
curl http://127.0.0.1:7860/api/auth/config
```

Si Azure / Microsoft Entra ID no quedó configurado, la respuesta será equivalente a:

```json
{"localAdminEnabled":false,"azureAd":{"enabled":false,"tenantId":null,"clientId":null}}
```

En ese estado el login institucional no se abre y la pantalla puede mostrar el flujo local. La causa esperada es que el contenedor no recibió `AZURE_AD_TENANT_ID` o `AZURE_AD_CLIENT_ID`.

Para habilitar Azure se deben configurar en `.env`:
...
La respuesta esperada debe indicar `azureAd.enabled` en `true`.

La Redirect URI se configura en Microsoft Entra ID, no en `.env`. Debe coincidir exactamente con la URL usada en el navegador. Ejemplos:
...
`http://localhost:17860` aplica únicamente a pruebas con túnel SSH o VS Code Ports. En producción debe usarse la URL HTTPS institucional definitiva.

### Acceso desde equipo cliente

`127.0.0.1` dentro del servidor significa el servidor. `127.0.0.1` en el navegador de Windows significa el equipo Windows. Si se valida desde VS Code conectado por SSH, usar **Ports / Puertos** para reenviar el puerto publicado por Docker.

Ejemplo con puerto alterno:
...
Con esa sesión abierta, navegar a `http://127.0.0.1:17860`.
...
| Azure AD falla | Revisar `AZURE_AD_TENANT_ID`, `AZURE_AD_CLIENT_ID`, dominios, grupos, roles, Redirect URI y `/api/auth/config`. |
| Frontend muestra errores de API | Revisar `VITE_API_BASE_URL`, CORS y backend. |
| Cambio de `.env` no aplica | Recrear contenedor con `docker compose up -d --force-recreate aurora`. |
| Puerto `7860` ocupado | Revisar `ss -ltnp | grep ':7860'`; liberar puerto o usar `HOST_PORT` temporal. |
| Navegador local no abre `127.0.0.1` | Usar IP/DNS del servidor o túnel SSH/VS Code Ports. |
