...
Este documento registra una revisión técnica y funcional básica de la aplicación Aurora. El alcance incluyó lectura de estructura, identificación de scripts, rutas API, variables de entorno, fuentes de datos, ejecución de comandos disponibles y validación manual de endpoints principales.

No se cambió la lógica funcional del sistema. Durante la revisión se ejecutó `npm install` en raíz, `backend/` y `frontend/`; ese comando dejó cambios en `backend/package-lock.json` y `frontend/package-lock.json`. También se ejecutó `npm run build` en `frontend/`, que regeneró el contenido de `frontend/dist`.
...
| Carpetas revisadas | raíz, `backend/`, `frontend/`, `docs/`, `scripts/` |
...
| `docs/` | Documentación técnica existente, incluyendo endpoints, estrategia de pruebas, Oracle y SSO. |

Variables de entorno detectadas en código: `PORT`, `NODE_ENV`, `ENABLE_STARTUP_WARMUP`, `CORS_ORIGIN`, `FORMATOS_BASE_URL`, `AUTH_*`, `AZURE_AD_*`, `AZURE_*`, `ORACLE_*`, `VITE_API_BASE_URL`, `VITE_DEV_API_TARGET`, `VITE_DEV_PORT`, `API_BASE_URL`.
...
| Método | Ruta | Protección | Resultado de revisión |
|---|---|---|---|
...
| POST | `/api/auth/azure-ad` | Pública con rate limit | No validada; no hay Azure AD configurado en esta revisión. |
...
Las pruebas automatizadas frontend pasaron: 4 archivos y 79 pruebas. La cobertura observada se concentra en reglas de formulario Aurora/Celeste y reglas de estado de actuaciones.

El build fue satisfactorio para esta etapa de revisión. La única observación fue la advertencia de Vite por un chunk JavaScript de 607.35 kB después de minificación. No bloquea el build, pero conviene revisar carga diferida de rutas o separación de chunks antes de despliegues con mayor tráfico.
