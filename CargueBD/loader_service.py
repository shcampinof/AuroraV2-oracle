...
SOURCES = {
    ...
    "ponal": SourceConfig(
        ...
        label="PONAL",
        table="PONAL",
        default_file="CONSOLIDADO_PPL_REGIONES.xlsx",
...
def default_path_for(source):
    config = SOURCES[source]
    return Path(__file__).resolve().parent / config.default_file
...
def main(argv=None):
    ...
    parser.add_argument("--archivo", "--file", dest="file_path", help="Ruta del archivo Excel a cargar.")
    parser.add_argument("--no-etl", action="store_true", help="Carga staging sin ejecutar el procedimiento ETL.")
    args = parser.parse_args(argv)

    file_path = args.file_path or str(default_path_for(args.source))
    try:
        run_load(args.source, file_path, execute_etl=not args.no_etl)
        return 0
